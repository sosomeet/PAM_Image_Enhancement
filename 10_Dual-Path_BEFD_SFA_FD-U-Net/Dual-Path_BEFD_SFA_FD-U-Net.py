...
# ------------------------------------------------------------
# E4: Dual-path boundary-detail branch
# ------------------------------------------------------------
#
# E4 keeps the complete E3 main context path unchanged and adds a
# lightweight second path specialized for the center LOW Y-Z plane.
#
# Main context path:
...
#
# AFF is intentionally not used in E4; it is reserved for E5.
# This keeps E4 as a controlled ablation of the dual-path addition.
#
...
# reducing the chance that the newly added detail path dominates the
# already-trained-style context reconstruction pathway at startup.
...
MODEL_ROOT = Path("./10_Dual-Path_BEFD_SFA_FD-U-Net/models_enhancement")
LATEST_MODEL_DIR = MODEL_ROOT / "latest"
CHECKPOINT_DIR = MODEL_ROOT / "checkpoints"
FINAL_MODEL_DIR = MODEL_ROOT / "final"

OUTPUT_ROOT = Path("./10_Dual-Path_BEFD_SFA_FD-U-Net/outputs")
...
class BoundaryConvBlock(nn.Module):
    """
    Lightweight two-convolution block for the E4 boundary-detail path.
...
class BoundaryDetailPath(nn.Module):
    """
    Lightweight E4 boundary-detail pathway.
...
class DualPathFullyDenseSFABEFDUNet(nn.Module):
    """
    E4 model: Dual-path Fully Dense U-Net + SFA + BEFD for 3-adjacent
    Y-Z PAM image enhancement.

    Controlled ablation relative to E3:
      - The complete E3 main context pathway is retained.
      - A lightweight boundary-detail pathway is added.
      - Gated boundary fusion is applied at Decoder 4, Decoder 3,
        and Decoder 1.
      - AFF is still disabled and reserved for E5.
    ...
    def __init__(
        ...
        # -------------------------
        # E4 lightweight boundary-detail path
        # -------------------------
    ...
    def forward(
        ...
        # ----------------------------------------------------
        # Decoder with E4 gated dual-path fusion
        # ----------------------------------------------------
        ...
        # Decoder 2 intentionally remains unchanged for the E4 ablation.
        d2 = self.upconv2(d3)
        d2 = self.decoder2(
...
def save_epoch_map(
    ...
    figure.suptitle(
        (
            "Dual-Path Fully Dense + SFA + BEFD U-Net Train MAP | "
            f"{volume_key} | "
            f"Epoch {epoch}"
        ),
        fontsize=14,
...
def save_checkpoint(
    ...
    checkpoint_data = {
        ...
        "model_name": "DualPathFullyDenseSFABEFDUNet",
...
def save_loss_curve(
    ...
    plt.title(
        "Dual-Path Fully Dense + SFA + BEFD U-Net Training Loss"
    )
...
def train() -> None:
    ...
    model = DualPathFullyDenseSFABEFDUNet(
    ...
    print(
        "  Architecture           : "
        "Dual-Path Fully Dense U-Net + SFA + BEFD (E4)"
    )
