...
# ------------------------------------------------------------
# E3: BEFD (Boundary Enhancement + Feature Denoising)
# ------------------------------------------------------------
#
...
MODEL_ROOT = Path("./09_BEFD_SFA_FD-U-Net/models_enhancement")
LATEST_MODEL_DIR = MODEL_ROOT / "latest"
CHECKPOINT_DIR = MODEL_ROOT / "checkpoints"
FINAL_MODEL_DIR = MODEL_ROOT / "final"

OUTPUT_ROOT = Path("./09_BEFD_SFA_FD-U-Net/outputs")
...
class FullyDenseSFABEFDUNet(nn.Module):
    """
    E3 model: Fully Dense U-Net + SFA + BEFD for 3-adjacent Y-Z PAM
    image enhancement.

    Relative to E2:
      - Same input/output definition.
      - Same Fully Dense U-Net backbone.
      - Same SFA modules after Encoder 3, Encoder 4, Bottleneck.
      - Adds BEFD Boundary Enhancement from the center LOW plane.
      - Adds BEFD-style Feature Denoising to skip features e1/e2/e3.
      - Keeps standard concatenation skips; AFF is reserved for E5.
      - No dual-path yet; that is reserved for E4.
      - Same linear output and L1 training loss.

    Boundary Enhancement:
        Center LOW [B, 1, 208, 512]
            -> fixed Sobel edge attention
            -> bilinear resize
    ...
    Feature Denoising:
        e1 -> FD1 -> skip to Decoder 1
        e2 -> FD2 -> skip to Decoder 2
        e3 -> FD3 -> skip to Decoder 3
        e4 remains the direct skip to Decoder 4.
    ...
    def __init__(
        ...
        # Linear output, identical to E0/E1/E2.
        self.output_conv = nn.Conv2d(
            c1,
            out_channels,
            kernel_size=1,
    ...
    def forward(
        ...
        # Center LOW plane corresponding to the HIGH[x, :, :] target.
        center = x[:, 1:2, :, :]

        # Fixed Sobel-derived BEFD edge attention.
        # [B, 1, 208, 512]
        ...
        # Encoder 3 + SFA + BEFD boundary enhancement.
        e3 = self.encoder3(
            self.pool2(e2)
        )
        e3 = self.sfa3(e3)
        ...
        # Encoder 4 + SFA + BEFD boundary enhancement.
        e4 = self.encoder4(
            self.pool3(e3)
        )
        e4 = self.sfa4(e4)
        ...
        # Bottleneck + SFA + BEFD boundary enhancement.
        ...
        # BEFD feature denoising on the first three skip features.
        skip1 = self.denoise1(e1)
        skip2 = self.denoise2(e2)
        skip3 = self.denoise3(e3)

        # Decoder 4 uses direct e4 skip, matching the three-block BEFD
        # design on the shallower low-level skip features.
        ...
        d3 = self.upconv3(d4)
        d3 = self.decoder3(
            torch.cat([d3, skip3], dim=1)
        )

        d2 = self.upconv2(d3)
        d2 = self.decoder2(
            torch.cat([d2, skip2], dim=1)
        )

        d1 = self.upconv1(d2)
        d1 = self.decoder1(
            torch.cat([d1, skip1], dim=1)
        )
...
def save_epoch_map(
    ...
    figure.suptitle(
        (
            "Fully Dense + SFA + BEFD U-Net Train MAP | "
            f"{volume_key} | "
            f"Epoch {epoch}"
        ),
        fontsize=14,
...
def save_checkpoint(
    ...
    checkpoint_data = {
        ...
        "model_name": "FullyDenseSFABEFDUNet",
...
def save_loss_curve(
    ...
    plt.title(
        "Fully Dense + SFA + BEFD U-Net Training Loss"
    )
...
def train() -> None:
    ...
    print(
        "PAM 3-ADJACENT Y-Z "
        "FULLY DENSE + SFA + BEFD U-NET TRAINING"
    )
    ...
    print(
        "  Dual-path              : None (reserved for E4)"
    )

    print(
        "  AFF                    : None (reserved for E5)"
    )
