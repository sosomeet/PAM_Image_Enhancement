# -*- coding: utf-8 -*-
"""
E9 PAM image enhancement training script.
...
Design goals:
    ...
    4. Limit hallucination with conservative gated residual injection.
    5. Keep the model practical for an RTX 3060 Ti.
...
class ModelConfig:
    def __init__(
        ...
        refine_gate_bias_init: float = -2.0,
...
class TrainConfig:
    def __init__(self) -> None:
        ...
        # ------------------------------------------------------------
        # Composite structure-aware loss
        # Final-stage weights are normalized to 1.0:
        #   15% L1 + 70% SSIM + 15% boundary gradient loss
        # SSIM is intentionally dominant because structural similarity is the
        # primary optimization target in the current PAM enhancement task.
        # ------------------------------------------------------------
        self.final_l1_weight = 0.15
        self.final_ssim_weight = 0.70
        self.final_boundary_weight = 0.15

        # Stage-1 deep supervision remains lighter so it stabilizes training
        # without overpowering the final prediction objective.
        self.coarse_loss_weight = 0.20
        self.coarse_l1_weight = 0.30
        self.coarse_ssim_weight = 0.70
...
class SaveConfig:
    def __init__(self) -> None:
        self.run_root = Path("./12_Residual_Edge_Wavelet_MultiStage")
...
class MultiStageStructureLoss(nn.Module):
    """
    SSIM-dominant composite loss for PAM vessel enhancement.

    Final-stage objective:
        0.15 * L1
      + 0.70 * SSIM loss
      + 0.15 * boundary gradient loss

    Stage-1 coarse deep supervision:
        0.20 * (0.30 * coarse L1 + 0.70 * coarse SSIM loss)

    Total:
        final_composite + coarse_weight * coarse_composite

    SSIM receives the largest weight by design. Boundary supervision is applied
    to the final prediction only, because the final E9 refinement stage is the
    component responsible for recovering sharp vessel boundaries.
    """

    def __init__(
        self,
        final_l1_weight: float = 0.15,
        final_ssim_weight: float = 0.70,
        final_boundary_weight: float = 0.15,
        coarse_weight: float = 0.20,
        coarse_l1_weight: float = 0.30,
        coarse_ssim_weight: float = 0.70,
...
def save_training_map(
    ...
    fig.suptitle(
        f"E9 PAM Enhancement | Epoch {epoch:03d} | 90° clockwise"
    )
    fig.tight_layout()
...
def model_metadata(samples_per_epoch: int) -> dict[str, Any]:
    return {
        ...
        "loss": (
            "0.15*final_L1 + 0.70*final_SSIM_loss + "
            "0.15*final_boundary + 0.20*(0.30*coarse_L1 + 0.70*coarse_SSIM_loss)"
        ),
...
def save_loss_curve(history: list[HistoryRow]) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("E9 PAM SSIM-Dominant Structure-Aware Training Loss")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def train() -> None:
    ...
    print("=" * 92)
    print("E9 PAM ENHANCEMENT — RESIDUAL + EDGE + HAAR WAVELET + 2-STAGE REFINEMENT")
    print("=" * 92)
